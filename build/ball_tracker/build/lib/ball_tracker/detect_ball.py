...
class DetectBall(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info('Looking for the ball...')     #take image as data (/image_in is remapped to camera/image_raw in terminal)
    ...
    def callback(self,data):
        try:
            cv_image = self.bridge.imgmsg_to_cv2(data, "bgr8")     #convert ros camera img to opencv image using bridge
        ...
        try:
            ...
            keypoints_norm, out_image, tuning_image = proc.find_circles(cv_image, self.tuning_params)    #use functions to get images for publishing

            img_to_pub = self.bridge.cv2_to_imgmsg(out_image, "bgr8")
            img_to_pub.header = data.header
            self.image_out_pub.publish(img_to_pub)   #publish to /image_out topic

            img_to_pub = self.bridge.cv2_to_imgmsg(tuning_image, "bgr8")
            img_to_pub.header = data.header
            self.image_tuning_pub.publish(img_to_pub)   #publish to /image_tuning topic
